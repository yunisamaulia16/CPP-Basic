...
int main(){//main program type interger
	int pilihan,hargaobat,banyak,hasil=0;//masukan jenis input variabel
	char pilihanobat,yesNo;//menyimpan karakter variabel
	int diskon = 10; //memasukan jenis input
	...
	do{//melakukan perulangan atau looping
		...
		switch(pilihan){//mengecek nilai pilihan
			case 1://jika pilihan bernilai 1
				...
				cout<<"E. Komix         : Rp.30.000,00"<<endl;//"E. Komix         : Rp.30.000,00"
				
				cout<<"MASUKAN PILIHAN OBAT : "<<endl;//cetak kalimat "MASUKAN PILIHAN OBAT : " 
				cin>>pilihanobat;//masukan pilihan obat
				
				if (pilihanobat == 'A' || pilihanobat =='a' ){//jika pilihan obat adalah 'A' atau 
		...
		x++;//menambah nilai variabel yang sudah di tetapkan
	} while(yesNo == 'Y' || yesNo == 'y');//melakukan mengulangan jika statement bernilai y atau benar, jika salah maka akan berakhir
	...
	for(int i=0;i<x;i++){ // for i = membaca i dengan variabel 0, i++ melakukan pengulangan sejumlah variabe yang ditentukan
		hasil=hasil+totalSeluruhObat[i];//hitung hasil
	}
